...
class CenterBluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    
    let completition: ((data: String) -> ())
    
    var didConnectBlock: (() -> ())?
    
    init(completionBlock:(data: String) -> ()) {
        self.completition = completionBlock
        super.init()
    ...
    func peripheral(peripheral: CBPeripheral, didUpdateValueForCharacteristic characteristic: CBCharacteristic, error: NSError?) {
        ...
        if stringFromData.isEqualToString("EOM") {
            ...
            if let resultData = String(data: data.copy() as! NSData, encoding: NSUTF8StringEncoding) {
                self.completition(data: resultData)
            }
